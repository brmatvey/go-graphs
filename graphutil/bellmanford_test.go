...
func TestBellmanFord(t *testing.T) {
	t.Run("test linear graph", func(t *testing.T) {
		...
		lengths, err := graphutil.BellmanFord(1, weightedGraph)
		if err != nil {
			t.Fatal("error must be nil")
		}
	...
	t.Run("test graph with negative edges", func(t *testing.T) {
		//   6   7    8       10
		// 1 ->2 -> 4 -> -> 7 -> -> -> 8
		// 1\      1    -1 /    /
		//   \->3 -> 5 ->/    /
		...
		if lengths[1] != 0 || lengths[2] != 6 || lengths[3] != 1 || lengths[4] != 13 ||
			lengths[5] != 2 || lengths[6] != 3 || lengths[7] != 1 || lengths[8] != 6 {
			t.Fatal("incorrect length")
		}
	})

	t.Run("test graph with negative circular dependencies", func(t *testing.T) {
		dependencies := map[int][]graph.Length[int]{
			1: {graph.NewLength(2, -1)},
			2: {graph.NewLength(1, -1)},
		}
		count := 0
		edgeKeyGen := func() int {
			count++
		...
		_, err = graphutil.BellmanFord(1, weightedGraph)
		if err == nil {
